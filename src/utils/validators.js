...
const validateRatingImages = (req, res, next) => {
              ...
              if (rating && rating.rating < 3) {
                if (!rating.photos || rating.photos.length === 0) {
                  errors.push({
                    ...
                    message: `Photos are required for ${component} with rating ${rating.rating} (below 3)`,
              ...
              if (rating && rating.rating < 3) {
                if (!rating.photos || rating.photos.length === 0) {
                  errors.push({
                    ...
                    message: `Photos are required for ${component.replace('_', ' ')} with rating ${rating.rating} (below 3)`,
                    location: `Floor ${floor.floor_number}, Flat ${flat.flat_number || flatIndex + 1}`
                  });
                }
              }
  ...
  if (errors.length > 0) {
    return res.status(400).json({
      success: false,
      message: 'Image validation failed - Photos required for ratings below 3',
      errors: errors
    });
  }
