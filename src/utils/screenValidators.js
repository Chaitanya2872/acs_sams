...
// =================== MULTI-COMPONENT RATING VALIDATION ===================
// For sending multiple component types in one request
const multiComponentRatingValidation = [
  ...
  // ✅ FIXED: _id is now optional - it will be auto-generated by backend if not provided
  ...
  // Custom validation: ratings < 3 must have detailed comments
...
const componentRatingValidation = [
  ...
  body('components.*._id')
    ...
    .trim()
    .isLength({ min: 1, max: 100 })
    .withMessage('Component ID must be 1-100 characters'),
  ...
  body('components.*.inspector_notes')
    .optional()
    .isString()
    .trim()
    .isLength({ max: 2000 })
...
// =================== COMPONENT UPDATE VALIDATION (PATCH) ===================
const componentUpdateValidation = [
  ...
  body().custom((requestBody) => {
    const allowedFields = ['name', 'rating', 'photo', 'condition_comment', 'inspector_notes'];
...
const parameterValidations = {
  ...
  blockId: param('blockId')
    ...
    .isLength({ min: 1, max: 50 })
    .withMessage('Block ID must be 1-50 characters'),
